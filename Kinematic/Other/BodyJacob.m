function Jb = BodyJacob( s, joint_type, q, L, h, g_s_INS, g_n_ef )
...
% Outputs:
% 'Jb' is the body geometric Jacobian.
...
Jb = zeros(6,num_joints);
for i = 1:num_joints
    dq = zeros(num_joints,1); dq(i) = 1;
    [ ~, ~, ~, V, ~ ] = kinematicStep( s, joint_type, q, dq, ddq, V0INS, dV0INS, L, h, g_s_INS, g_n_ef );
    Jb(:,i) = V(:,end);
end
