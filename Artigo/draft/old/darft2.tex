...
A powered controlled prosthesis requires less metabolic energy from the user for walking than purely passive because they are able to generate net power at the joints through DC Motors or pneumatic actuators \cite{Goldfarb2008} \cite{Bedard2002}. As they are able to supply positive power, the user could more easily realize tasks such as climbing stairs, walking uphill.
...
In \cite{Fakoorian2016} an Extended Kalman Filter is designed to estimate joint position, velocities and ground reaction forces. In the present case, an HGO approach will be deployed.
...
Considering that robot/prosthesis system has parametric uncertainties and angle measurement is subject to noise, a time-varying HGO design is proposed, similar to \cite{POH:2011}. While estimating in real-time the control signal noise, the adaptation law changes the observer gain to achieve a preferable trade-off between control noise and tracking performance. Global results are not pursued in this paper.
